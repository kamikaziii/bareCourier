...
{#snippet categoryRow(
  category: keyof typeof notificationPrefs.categories,
  titleMsg: () => string,
  descMsg: () => string,
)}
  <div class="grid grid-cols-4 gap-4 items-center">
    ...
    <div class="flex justify-center">
      <Checkbox
        checked={notificationPrefs.categories[category].inApp}
        disabled
      />
    </div>
    <div class="flex justify-center">
      <Checkbox
        ...
        onCheckedChange={(v) =>
          updateCategoryPref(category, "push", v === true)}
    ...
    <div class="flex justify-center">
      <Checkbox
        ...
        onCheckedChange={(v) =>
          updateCategoryPref(category, "email", v === true)}
...
>
  <input
    ...
    value={JSON.stringify(notificationPrefs)}
  />
  ...
  <Card.Root>
    <Card.Header>
      ...
      <Card.Description
        >{m.settings_notification_preferences_desc()}</Card.Description
      >
    </Card.Header>
    <Card.Content class="space-y-4">
      <!-- Header row -->
      <div
        class="grid grid-cols-4 gap-4 text-sm font-medium text-muted-foreground border-b pb-2"
      >
        <div></div>
      ...
      {@render categoryRow(
        "new_request",
        m.settings_category_new_request,
        m.settings_category_new_request_desc,
      )}
      {@render categoryRow(
        "schedule_change",
        m.settings_category_schedule_change,
        m.settings_category_schedule_change_desc,
      )}
      {#if role === "courier"}
        {@render categoryRow(
          "past_due",
          m.settings_category_past_due,
          m.settings_category_past_due_desc,
        )}
        {@render categoryRow(
          "daily_summary",
          m.settings_category_daily_summary,
          m.settings_category_daily_summary_desc,
        )}
      {/if}
      {@render categoryRow(
        "service_status",
        m.settings_category_service_status,
        m.settings_category_service_status_desc,
      )}
      ...
      <div class="flex items-center justify-between">
        <div>
          ...
          <p class="text-xs text-muted-foreground">
            {m.settings_working_days_only_desc()}
          </p>
        </div>
        <Switch
...
{#if role === "courier"}
  ...
  <Card.Root>
    <Card.Header>
      ...
      <Card.Description
        >{m.settings_automated_notifications_desc()}</Card.Description
      >
    </Card.Header>
    <Card.Content>
      <form
      ...
      >
        ...
        <div class="space-y-2">
          <Label for="pastDueReminderInterval"
            >{m.settings_past_due_reminder_interval()}</Label
          >
          <select
          ...
          <p class="text-xs text-muted-foreground">
            {m.settings_past_due_reminder_interval_desc()}
          </p>
        </div>
        ...
        <div class="space-y-4">
          <div class="flex items-center justify-between">
            <div>
              ...
              <p class="text-xs text-muted-foreground">
                {m.settings_daily_summary_desc()}
              </p>
            </div>
            <input
              type="hidden"
              name="dailySummaryEnabled"
              value={pastDueSettings.dailySummaryEnabled.toString()}
            />
            <Switch
          ...
          {#if pastDueSettings.dailySummaryEnabled}
            <div class="space-y-2">
              <Label for="dailySummaryTime"
                >{m.settings_daily_summary_time()}</Label
              >
              <Input
                id="dailySummaryTime"
