  ...
  async function updateMapContent(
    ...
    if (pickup) {
      ...
      pickupMarker.className =
        "size-6 bg-blue-500 rounded-full border-2 border-white shadow-lg flex items-center justify-center";
    ...
    if (delivery) {
      ...
      deliveryMarker.className =
        "size-6 bg-green-500 rounded-full border-2 border-white shadow-lg flex items-center justify-center";
...
<div class="space-y-2">
  ...
  {#if !hideFooter && (distanceKm !== null || (pickupCoords && deliveryCoords))}
    <div class="flex items-center justify-between">
      {#if distanceKm !== null}
        <span class="text-sm text-muted-foreground">
          {#if durationMinutes !== null}
            {m.map_distance_duration({
              km: formatDistance(distanceKm),
              minutes: String(Math.round(durationMinutes)),
            })}
          {:else}
      ...
      {#if !hideDirections && pickupCoords && deliveryCoords}
        <button
          type="button"
