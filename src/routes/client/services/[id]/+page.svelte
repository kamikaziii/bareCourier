<script lang="ts">
  ...
  const canReschedule = $derived.by(() => {
    ...
    if (!data.rescheduleSettings.allowed)
      return { allowed: false, reason: "disabled" };
    if (
      (service.reschedule_count || 0) >= data.rescheduleSettings.maxReschedules
    ) {
  ...
  async function handleDeclineReschedule() {
    ...
    try {
      const response = await fetch("?/declineReschedule", {
        method: "POST",
        body: formData,
      });
      const result = await response.json();
    ...
    <div class="flex gap-2">
      ...
      {#if service.request_status === "pending"}
        <Button
          variant="outline"
          size="sm"
          href={localizeHref(`/client/services/${service.id}/edit`)}
        >
          {m.action_edit()}
        </Button>
      {/if}
      ...
      {:else}
        ...
        <Card.Root class="border-orange-200 bg-orange-50">
          <Card.Content class="flex items-start gap-3 p-4">
            ...
            <div>
              <p class="font-medium text-orange-800">
                {m.client_reschedule_pending()}
              </p>
              <p class="text-sm text-orange-700 mt-1">
                {m.client_reschedule_pending_desc()}
              </p>
              <p class="text-sm text-orange-600 mt-2">
                {formatDate(service.pending_reschedule_date)}
                {#if service.pending_reschedule_time_slot}
          ...
          <Card.Content class="space-y-2">
            ...
            {#if service.calculated_price !== null}
              ...
              <div class="flex justify-between font-medium">
                ...
                <span class="text-lg"
                  >{formatCurrency(service.calculated_price)}</span
                >
              </div>
            {/if}
          </Card.Content>
...
<Dialog.Root bind:open={showRescheduleDialog}>
  <Dialog.Content class="sm:max-w-md">
    <Dialog.Header>
      ...
      <Dialog.Description
        >{m.client_request_reschedule_desc()}</Dialog.Description
      >
    </Dialog.Header>
    ...
    <div class="space-y-4 py-4">
      ...
      <p class="text-xs text-muted-foreground">
        {m.client_reschedule_too_late({
          hours: data.rescheduleSettings.minNoticeHours,
        })}
      </p>
    </div>
    ...
    <Dialog.Footer>
      ...
      <Button
        ...
        disabled={!rescheduleDate ||
          !rescheduleTimeSlot ||
          (rescheduleTimeSlot === "specific" && !rescheduleTime) ||
          rescheduleLoading}
      >
