<script lang="ts">
  ...
  function selectAllVisible() {
    batch.selectAll(
      sortedServices.filter((s) => s.status === "pending").map((s) => s.id),
    );
  }
  ...
  async function handleBatchReschedule() {
    if (!batchRescheduleDate || !batchRescheduleTimeSlot || !batch.hasSelection)
      return;
  ...
  async function loadWorkload() {
    ...
    workload = await calculateDayWorkload(
      data.supabase,
      data.profile.id,
      new Date(),
      settings,
  ...
  const sortedServices = $derived(
    sortByUrgency(filteredServices, pastDueConfig),
  );
</script>
      ...
      {:else}
        <Card.Root>
          <Card.Content class="flex items-center gap-3 p-4">
            <div
              class="size-8 rounded-full bg-blue-500/10 flex items-center justify-center"
            >
        ...
        <Card.Root>
          <Card.Content class="flex items-center gap-3 p-4">
            <div
              class="size-8 rounded-full bg-green-500/10 flex items-center justify-center"
            >
    ...
    {#if batch.selectionMode}
      ...
      >
        ...
        {#if batch.hasSelection}
          ...
          <Button
            size="sm"
            onclick={openBatchRescheduleDialog}
            disabled={batch.exceedsLimit}
          >
      ...
      {#if batch.exceedsLimit}
        <div class="rounded-md bg-orange-500/10 p-3 text-orange-600 text-sm">
          ...
          Please deselect {batch.selectedCount - batch.maxBatchSize} service(s) to
          continue.
        </div>
      {/if}
    {/if}
      ...
      {:else}
        {#each sortedServices as service (service.id)}
          <ServiceCard
            ...
            onClick={() => {
              goto(localizeHref(`/courier/services/${service.id}`));
            }}
          >
  ...
  <Dialog.Root bind:open={showBatchRescheduleDialog}>
    <Dialog.Content class="sm:max-w-md">
      ...
      <Dialog.Footer>
        <Button
          variant="outline"
          onclick={() => (showBatchRescheduleDialog = false)}
        ...
        <Button
          ...
          disabled={!batchRescheduleDate ||
            !batchRescheduleTimeSlot ||
            (batchRescheduleTimeSlot === "specific" && !batchRescheduleTime) ||
