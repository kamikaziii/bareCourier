<script lang="ts">
  ...
  let pricingModel = $state<PricingModel>(
    (data.pricing?.pricing_model as PricingModel) || "per_km",
  );
  // svelte-ignore state_referenced_locally
  ...
  // Sync pricing form state when data updates (after form submission + invalidateAll)
  $effect(() => {
    pricingModel = (data.pricing?.pricing_model as PricingModel) || "per_km";
    baseFee = data.pricing?.base_fee?.toString() || "0";
    perKmRate = data.pricing?.per_km_rate?.toString() || "0";
    zones =
      data.zones.length > 0
        ? data.zones.map((z) => ({
            min_km: z.min_km,
  ...
  let missingPriceCount = $derived(
    services.filter((s) => s.calculated_price === null).length,
  );
  ...
  async function loadServices() {
    ...
    for (const s of services) {
      ...
      totalRevenue += price;
      if (vatEnabled) {
        const vb = calculateVat(
          price,
          s.vat_rate_snapshot,
          s.prices_include_vat_snapshot,
        );
        totalNet += vb.net;
        totalVat += vb.vat;
        totalGross += vb.gross;
  ...
  function validateZones(): boolean {
    ...
    for (let i = 0; i < sortedZones.length; i++) {
      ...
      const originalIndex = zones.findIndex(
        (z) =>
          z.min_km === zone.min_km &&
          z.max_km === zone.max_km &&
          z.price === zone.price,
      );
      ...
      if (zone.min_km < 0) {
        errors.push({
          index: originalIndex,
          message: m.billing_zone_error_negative(),
        });
      }
      ...
      if (zone.max_km !== null && zone.min_km >= zone.max_km) {
        errors.push({
          index: originalIndex,
          message: m.billing_zone_error_min_max(),
        });
      }
      ...
      if (i < sortedZones.length - 1) {
        ...
        if (zone.max_km !== null && zone.max_km < nextZone.min_km) {
          errors.push({
            index: originalIndex,
            message: m.billing_zone_error_gap(),
          });
        }
      }
      ...
      if (i < sortedZones.length - 1) {
        ...
        if (zone.max_km === null || zone.max_km > nextZone.min_km) {
          errors.push({
            index: originalIndex,
            message: m.billing_zone_error_overlap(),
          });
        }
      }
    }
    ...
    if (unlimitedZones.length > 1) {
      ...
      errors.push({
        index: lastUnlimited,
        message: m.billing_zone_error_multiple_unlimited(),
      });
    }
  ...
  function exportClientCSV() {
    ...
    const rows = services.map((s) => {
      const priceColumns = vatEnabled
        ? (() => {
            const vb = calculateVat(
              s.calculated_price || 0,
              s.vat_rate_snapshot,
              s.prices_include_vat_snapshot,
            );
            return [
              formatCurrency(vb.net),
              formatCurrency(vb.vat),
              formatCurrency(vb.gross),
            ];
          })()
    ...
    rows.push([
      ...
      ...(vatEnabled
        ? [
            ...
            formatCurrency(totalStats.totalVat),
...
<div class="min-w-0 space-y-6">
        ...
        >
          <div class="space-y-2">
            ...
            >
              ...
              <option value="flat_plus_km"
                >{m.billing_model_flat_plus_km()}</option
              >
  ...
  <div class="space-y-4">
      ...
      <div class="flex flex-wrap gap-2">
        ...
        >
          ...
          <Button
            ...
            variant="outline"
            size="sm"
            disabled={recalculating || services.length === 0}
          >
            <Calculator class="mr-2 size-4" />
    ...
    {#if missingPriceCount > 0}
      <div
        class="rounded-md bg-amber-500/10 p-3 flex items-center gap-2 text-amber-600"
      >
            ...
            <table class="w-full">
              ...
              <tbody>
                {#each services as service (service.id)}
                  {@const vatBreakdown = vatEnabled
                    ? calculateVat(
                        service.calculated_price || 0,
                        service.vat_rate_snapshot,
                        service.prices_include_vat_snapshot,
                      )
                    : null}
                  <tr class="border-b">
                    ...
                    {#if vatEnabled && vatBreakdown}
                      <td class="px-4 py-3 text-right text-sm"
                        >{formatCurrency(vatBreakdown.net)}</td
                      >
                      <td
                        class="px-4 py-3 text-right text-sm text-muted-foreground"
                        >{formatCurrency(vatBreakdown.vat)}</td
                      >
                      <td class="px-4 py-3 text-right text-sm font-medium"
                        >{formatCurrency(vatBreakdown.gross)}</td
                      >
                    {:else}
                      <td class="px-4 py-3 text-right text-sm font-medium">
