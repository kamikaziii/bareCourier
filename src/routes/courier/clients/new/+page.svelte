<script lang="ts">
	...
	import { ArrowLeft, Package, Euro, ChevronDown, AlertTriangle } from '@lucide/svelte';
	...
	async function handleSubmit(e: Event) {
		...
		const response = await fetch(`${PUBLIC_SUPABASE_URL}/functions/v1/create-client`, {
			method: 'POST',
			headers: {
		...
		if (pricingSaveFailed) {
			...
			warning = 'Client created, but pricing configuration failed to save. You can add it later.';
		} else {
			success = m.clients_success();
		}
		loading = false;
	...
	<Card.Root>
		...
		<Card.Content>
			<form onsubmit={handleSubmit} class="space-y-4">
				...
				{/if}
				{#if warning}
					<div class="rounded-md bg-amber-500/10 p-3 text-sm text-amber-600 flex items-start gap-2">
				...
				<div class="flex gap-3 pt-4">
					<Button type="submit" disabled={loading || !name.trim() || !email.trim() || !password || password.length < 6}>
