<script lang="ts">
  ...
  function getWorkloadForService(
    service: ServiceWithClient,
  ): { workload: WorkloadEstimate; label: string } | null {
...
<div class="space-y-6">
    ...
    <div>
      ...
      <p class="text-muted-foreground">
        {data.pendingRequests.length === 1
          ? m.requests_count_one({ count: data.pendingRequests.length })
          : m.requests_count_other({ count: data.pendingRequests.length })}
      </p>
    </div>
  ...
  {#if batch.selectionMode}
    <div
      class="flex items-center gap-2 flex-wrap rounded-lg border bg-muted/50 p-2"
    >
      ...
      {#if batch.hasSelection}
        ...
        <Button
          ...
          disabled={batchLoading || batch.exceedsLimit}
        >
    ...
    {#if batch.exceedsLimit}
      <div class="rounded-md bg-orange-500/10 p-3 text-orange-600 text-sm">
        Maximum {batch.maxBatchSize} services per batch. Currently selected: {batch.selectedCount}.
        Please deselect {batch.selectedCount - batch.maxBatchSize} service(s) to continue.
      </div>
    {/if}
  {/if}

  {#if data.pendingRequests.length === 0}
    <EmptyState icon={Inbox} title={m.requests_no_pending()} />
  {:else}
            ...
            >
              ...
              <div class="space-y-3 flex-1">
                ...
                <div class="flex items-center gap-2">
                  <a
                    href={localizeHref(
                      `/courier/clients/${service.profiles.id}`,
                    )}
                ...
                {#if service.requested_date || service.requested_time_slot}
                  <div class="flex items-center gap-2">
                    <Badge variant="outline"
                      >{m.requests_requested_schedule()}</Badge
                    >
                    <span class="text-sm">
              ...
              <div class="flex flex-wrap gap-2">
                ...
                <Button
                  size="sm"
                  variant="outline"
                  onclick={() => openSuggestDialog(service)}
                >
                  {m.requests_suggest()}
                </Button>
                <Button
    ...
    <div class="space-y-4">
      <div>
        ...
        <p class="text-muted-foreground text-sm">
          {m.courier_pending_reschedules_desc()}
        </p>
      </div>
            ...
            <Card.Content class="pt-6">
              ...
              >
                ...
                <div class="flex flex-wrap gap-2">
                  <Button
                    size="sm"
                    onclick={() => handleApproveReschedule(service.id)}
                  >
                  ...
                  <Button
                    size="sm"
                    variant="destructive"
                    onclick={() => openDenyRescheduleDialog(service)}
                  >
          ...
          <div class="mt-3 p-3 bg-blue-50 dark:bg-blue-950/30 rounded-lg">
            <div class="flex items-center gap-2 text-sm">
              ...
              <span class="text-blue-600 font-medium">
                {m.workload_next_compatible()}: {formatRequestDate(
                  data.nextCompatibleDay.date,
                )}
              </span>
            </div>
          </div>
...
<Dialog.Root bind:open={showDenyRescheduleDialog}>
  <Dialog.Content>
    ...
    <Dialog.Footer>
      <Button
        variant="outline"
        onclick={() => (showDenyRescheduleDialog = false)}
        disabled={loading}
      >
        {m.action_cancel()}
      </Button>
