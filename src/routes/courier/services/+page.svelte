<script lang="ts">
  ...
  function selectAllVisible() {
    batch.selectAll(
      filteredServices.filter((s) => s.status === "pending").map((s) => s.id),
    );
  }
  ...
  const filteredServices = $derived(
    sortByUrgency(
      services.filter((s) => {
        ...
        if (clientFilter !== "all" && s.client_id !== clientFilter)
          return false;
        if (searchQuery) {
          const query = searchQuery.toLowerCase();
          const matchesClient = s.profiles?.name?.toLowerCase().includes(query);
          const matchesPickup = s.pickup_location
            ?.toLowerCase()
            .includes(query);
          const matchesDelivery = s.delivery_location
            ?.toLowerCase()
            .includes(query);
...
<PullToRefresh>
  <div class="min-w-0 space-y-6">
            ...
            <DropdownMenu.Sub>
              <DropdownMenu.SubTrigger>
                ...
                {clientFilter !== "all"
                  ...
                  : m.services_all_clients()}
              </DropdownMenu.SubTrigger>
              <DropdownMenu.SubContent>
                <DropdownMenu.RadioGroup bind:value={clientFilter}>
                  <DropdownMenu.RadioItem value="all"
                    >{m.services_all_clients()}</DropdownMenu.RadioItem
                  >
                  {#each clients as client (client.id)}
                    <DropdownMenu.RadioItem value={client.id}
                      >{client.name}</DropdownMenu.RadioItem
                    >
                  {/each}
    ...
    {#if formWarning}
      <div
        class="rounded-md bg-amber-500/10 p-3 flex items-center justify-between text-amber-600"
      >
        <span class="text-sm">
