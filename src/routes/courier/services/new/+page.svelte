<script lang="ts">
  ...
  const selectedServiceType = $derived(
    data.serviceTypes.find(
      (st: ServiceType) => st.id === selectedServiceTypeId,
    ),
  );
  ...
  async function loadData() {
    ...
    const [clientsResult, feesResult, settings] = await Promise.all([
      data.supabase
        .from("profiles")
        .select(
          "id, name, default_pickup_location, default_pickup_lat, default_pickup_lng, default_service_type_id",
        )
        .eq("role", "client")
        .eq("active", true)
        .order("name"),
  ...
  function handleFormSubmit() {
    ...
    return async ({
      ...
      result: {
        type: string;
        data?: { error?: string; success?: boolean; warning?: string };
      };
    }) => {
          ...
          <div class="flex gap-2">
            ...
            <Button
              ...
              disabled={formLoading ||
                ...
                (isTypePricingMode && !selectedServiceTypeId)}
            >
