  ...
  async function runDiagnostics() {
    ...
    try {
      ...
      if (subscription) {
        ...
        diagnostics["Subscription endpoint"] =
          subJson.endpoint?.substring(0, 50) + "...";
        diagnostics["Subscription has keys"] = !!(
          subJson.keys?.p256dh && subJson.keys?.auth
        );

        // Check if endpoint is Apple's push service
        diagnostics["Is Apple Push"] =
          subJson.endpoint?.includes("apple") ||
          subJson.endpoint?.includes("push.apple.com") ||
          false;
        diagnostics["Is FCM"] =
          subJson.endpoint?.includes("fcm.googleapis.com") || false;
        diagnostics["Is Mozilla"] =
          subJson.endpoint?.includes("mozilla") || false;
    ...
    try {
      ...
      if (error) {
        ...
        if (subs && subs.length > 0) {
          diagnostics["DB subscription endpoint"] =
            subs[0].endpoint.substring(0, 50) + "...";
          diagnostics["DB subscription created"] =
            subs[0].created_at || "unknown";
        }
      }
    } catch (error) {
  ...
  async function sendTestPush() {
    ...
    try {
      ...
      const response = await fetch(
        `${PUBLIC_SUPABASE_URL}/functions/v1/send-notification`,
        {
          method: "POST",
          headers: {
    ...
    <div class="space-y-2 font-mono text-sm">
      {#each Object.entries(diagnostics) as [key, value] (key)}
        <div class="flex justify-between border-b py-1 gap-2">
          ...
          <span
            class="text-xs text-right {value === true
              ? 'text-green-500'
              : value === false
                ? 'text-red-500'
                : ''}"
...
{#if subscriptionDetails}
  <Card.Root>
    ...
    <Card.Content>
      <pre
        class="text-xs bg-muted p-3 rounded overflow-auto max-h-48">{subscriptionDetails}</pre>
    </Card.Content>
  </Card.Root>
{/if}
...
<Card.Root>
  ...
  <Card.Content class="space-y-4">
    ...
    {#if testResult}
      <pre
        class="text-xs bg-muted p-3 rounded overflow-auto max-h-64 whitespace-pre-wrap">{testResult}</pre>
    {/if}
  </Card.Content>
</Card.Root>
...
<Card.Root>
  ...
  <Card.Content class="text-sm space-y-2">
    <p>
      <strong>Local works, server doesn't?</strong> → Issue with subscription or server.
    </p>
    <p>
      <strong>Neither works?</strong> → Check iOS Settings → AS Estafetagem → Notifications.
    </p>
    <p>
      <strong>DB count is 0?</strong> → Re-enable push in Notifications tab.
    </p>
  </Card.Content>
</Card.Root>
