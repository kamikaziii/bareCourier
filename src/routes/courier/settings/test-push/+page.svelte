<script lang="ts">
	...
	async function runDiagnostics() {
		...
		try {
			const { data: subs, error } = await data.supabase
		...
		try {
			...
			const response = await fetch(`${PUBLIC_SUPABASE_URL}/functions/v1/send-push`, {
				...
				body: JSON.stringify({
					...
					url: '/courier/settings/test-push'
				})
			});
...
<div class="container max-w-2xl py-6 space-y-6">
	<h1 class="text-2xl font-bold">Push Notification Diagnostics</h1>
			...
			<div class="space-y-2 font-mono text-sm">
				{#each Object.entries(diagnostics) as [key, value] (key)}
					<div class="flex justify-between border-b py-1">
						...
						<span class={value === true ? 'text-green-500' : value === false ? 'text-red-500' : ''}>
							{String(value)}
						</span>
					</div>
				{/each}
			</div>
			<Button onclick={runDiagnostics} disabled={loading} class="mt-4">
	...
	<Card.Root>
		...
		<Card.Content class="space-y-4">
			<div class="flex gap-2">
				<Button onclick={showLocalNotification} disabled={loading} variant="outline">
					Test Local Notification
				</Button>
				<Button onclick={sendTestPush} disabled={loading}>
					Send Server Push
				</Button>
			</div>
			{#if testResult}
	...
	<Card.Root>
		<Card.Header>
			<Card.Title>Debugging Tips for iOS</Card.Title>
		</Card.Header>
		<Card.Content class="text-sm space-y-2">
			<p>1. <strong>Local notification works but server push doesn't?</strong> → Issue is with push subscription or server-side.</p>
			<p>2. <strong>Neither works?</strong> → Check iOS Settings → bareCourier → Notifications.</p>
			<p>3. <strong>"Is Apple Push" shows false?</strong> → iOS may not be using APNs correctly.</p>
			<p>4. <strong>DB subscription count is 0?</strong> → Re-enable push in settings.</p>
			<p>5. <strong>Connect Safari Web Inspector:</strong> Mac Safari → Develop → [Your iPhone] → Service Worker</p>
		</Card.Content>
	</Card.Root>
</div>
