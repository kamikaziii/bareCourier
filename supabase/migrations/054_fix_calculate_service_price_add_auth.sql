-- Fix HIGH: calculate_service_price has NO authentication check
-- Any user (even anonymous) could query pricing information for any client
...
RETURNS TABLE(total_price numeric, price_breakdown jsonb)
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path = ''
AS $$
...
BEGIN
  ...
  IF v_user_id IS NULL THEN
    RETURN QUERY SELECT 0::decimal(10,2), '{"error": "Not authenticated"}'::jsonb;
    RETURN;
  END IF;

  -- Verify user is courier (only courier should calculate prices)
  SELECT role INTO v_user_role FROM public.profiles WHERE id = v_user_id;

  IF v_user_role IS NULL OR v_user_role != 'courier' THEN
    RETURN QUERY SELECT 0::decimal(10,2), '{"error": "Unauthorized - courier role required"}'::jsonb;
    RETURN;
  END IF;
...
COMMENT ON FUNCTION calculate_service_price IS 'Calculates service price for a client. Requires courier role.';
