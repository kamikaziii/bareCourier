-- Fix CRITICAL: bulk_reschedule_services impersonation vulnerability
-- The function was accepting p_user_id parameter and using COALESCE(p_user_id, auth.uid())
-- This allowed any authenticated user to impersonate the courier by passing their UUID
-- Fix: Always use auth.uid() for authentication, ignore p_user_id parameter
...
COMMENT ON FUNCTION bulk_reschedule_services IS 'Bulk reschedule services atomically. SECURITY: Always uses auth.uid() for authentication, ignores p_user_id parameter.';
