-- Fix #261: RPC Exception Handler Prevents Proper Transaction Rollback
-- The previous implementations caught exceptions with EXCEPTION WHEN OTHERS and returned JSON
-- instead of propagating errors. This caused partial commits when later operations failed.
-- This migration fixes all three reschedule RPC functions to re-raise after logging.

-- 1. Fix reschedule_service
...
COMMENT ON FUNCTION reschedule_service(uuid, date, text, text, text, text, text) IS 'Courier reschedules a service directly. Creates notification for client and records history. Fixed: Re-raises exceptions for proper transaction rollback.';


-- 2. Fix client_approve_reschedule
...
COMMENT ON FUNCTION client_approve_reschedule(uuid) IS 'Client approves a courier-suggested reschedule. Applies suggested date/time as scheduled. Fixed: Re-raises exceptions for proper transaction rollback.';


-- 3. Fix client_deny_reschedule
...
COMMENT ON FUNCTION client_deny_reschedule(uuid, text) IS 'Client denies a courier-suggested reschedule. Clears suggestion, keeps original schedule. Fixed: Re-raises exceptions for proper transaction rollback.';
