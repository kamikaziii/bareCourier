-- Migration: Fix profile update trigger - remove email column check and allow timezone
--
-- Fixes two bugs in check_client_profile_update_fields():
--
-- 1. (#334 CRITICAL) The function checked NEW.email / OLD.email, but the profiles
--    table has NO email column. This caused a runtime crash on ANY client profile
--    update: 'record "new" has no field "email"'.
--
-- 2. (#335) The function blocked clients from changing timezone, but timezone is
--    a user preference (used by NotificationsTab for quiet hours). Clients should
--    be allowed to set their own timezone.
--
-- Uses CREATE OR REPLACE to update the function in-place; the existing trigger
-- already references it, so no trigger recreation is needed.
...
-- Update the function comment to reflect the new allowed fields
